...
if [[ "$JOB_STATE" == COMPLETED* ]] && [ ! -f $D/results/activations/stage1_activations.h5 ] && ! squeue -u ydon268 --name=ecg_extract -h | grep -q .; then
...
if [ -f $D/results/activations/stage3_activations.h5 ] && [ -f $D/results/activations/beat_meta.parquet ] && ! squeue -u ydon268 --name=ecg_sae -h | grep -q . && ! ls $D/results/sae/stage1_r4_k8.pt 2>/dev/null; then
...
if [ "$N_SAE" -eq 36 ] && ! squeue -u ydon268 --name=ecg_interp -h | grep -q . && ! ls $D/results/interp/stage1_r4_k8_interp_binary.parquet 2>/dev/null; then
